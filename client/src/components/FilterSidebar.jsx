...
const FilterSidebar = ({showFilterPhone, setShowFilterPhone, filters, setFilters}) => {
    ...
    const [expandendSections, setExpandendSections] = useState({
        platform:true,
        price:true,
        followers:true,
        niche:true,
    ...
    const toggleSection = (section) => {
        setExpandendSections((prev)=>({...prev, [section]:!prev[section]}))
    }
            ...
            <div className='p-4 space-y-6 sm:max-h[calc(100vh-200px)] overflow-y-scroll no-scrollbar'>
                ...
                <div>
                    <button onClick={()=> toggleSection("platform")} className='flex items-center justify-between w-full mb-3'>
                        ...
                        <ChevronDown className={`size-4 transition-transform ${expandendSections.platform ? "rotate-180":" "}`}/>
                    </button>
                    {expandendSections.platform && (
                ...
                <div>
                    <button onClick={()=> toggleSection("price")} className='flex items-center justify-between w-full mb-3'>
                        ...
                        <ChevronDown className={`size-4 transition-transform ${expandendSections.price ? "rotate-180":" "}`}/>
                    </button>
                    {expandendSections.price && (
                ...
                <div>
                    <button onClick={()=> toggleSection("followers")} className='flex items-center justify-between w-full mb-3'>
                        ...
                        <ChevronDown className={`size-4 transition-transform ${expandendSections.followers ? "rotate-180":" "}`}/>
                    </button>
                    {expandendSections.followers && (
                        <select
                ...
                {/*Niche Filter  */}
                 <div>
                    <button onClick={()=> toggleSection("niche")} className='flex items-center justify-between w-full mb-3'>
                        ...
                        <ChevronDown className={`size-4 transition-transform ${expandendSections.niche ? "rotate-180":" "}`}/>
                    </button>
                    {expandendSections.niche && (
                        <select
                        value={filters.niche || ""}
                ...
                {/* Verification filter */}
                 <div>
                    <button onClick={()=> toggleSection("status")} className='flex items-center justify-between w-full mb-3'>
                        ...
                        <ChevronDown className={`size-4 transition-transform ${expandendSections.status ? "rotate-180":" "}`}/>
                    </button>
                    {expandendSections.status && (
                        <div className='space-y-3'>
                            ...
                            <label className='flex items-center space-x-2 cursor-pointer'>
                                ...
                                <span>Monetized account only</span>
                            </label>                            
