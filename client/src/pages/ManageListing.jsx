...
const ManageListing = () => {
    ...
    const handleImageUpload = async (event) =>{
        ...
        if(files.length + formData.images.length > 5) return toast.error("You can only upload 5 images");
                ...
                <form onSubmit={handleSubmit} className='space-y-8'>
                    ...
                    <Section title = "Account Metrics">
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
                            ...
                            <InputField label='Primary Audience Country' value={formData.country} placeholder='United States' onChange={(v)=>handleInputChange('country',v)} required={true}/>   
                    ...
                    {/* Pricing  */}
                    <Section title='Pricing & Descriptions'>
                    ...
                    <div className='flex justify-end gap-3 text-sm'>
                        ...
                        <button type='submit' className='px-6 py-2 border text-white border-gray-300 rounded-lg bg-indigo-600 hover:bg-indigo-700 transition-colors'>
...
const SelectField = ({label, value, onChange, options, required = false})=>(
    <div className=''>
        <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
        <select name="" id="" value={value} onChange={(e)=>onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300' required={required}>
