...
export const addCredential = async (req, res) => {
    try{
        ...
        return res.json({message: "Credential added successfully", listing})
...
export const purchaseAccount = async (req, res) => {
    try{
        ...
        if(listing.ownerId === userId){
            return res.status(404).json({message: "You cannot purchase your own listing"});
       ...
       const session = await stripeInstance.checkout.sessions.create({
            ...
            line_items: [
                {
                    price_data: {
                        currency: 'usd', // Currency can be dynamic based on your requirements
                        product_data: {
